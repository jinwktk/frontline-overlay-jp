...
const ui_fold_cache = store.appConfig.ui_fold_cache ?? {}
...
const updateCache = () => {
  const _ui_fold_cache = store.appConfig.ui_fold_cache ?? {} // refresh to avoid mutation
  _ui_fold_cache[props.cardKey] = folded.value
  store.setAppConfig({
    ...store.appConfig,
    ui_fold_cache: _ui_fold_cache
  })
}
